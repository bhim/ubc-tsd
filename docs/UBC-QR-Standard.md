...
The `Item Id` represents the specific Connector and must strictly adhere to the following breakdown to ensure unique identification across the network. 

**Format:**

```
<country code><separator><operator id><separator><charging station id><separator><charging point id><separator><connector id>
```
...
| Operator ID | CPO code | 3 Alphanumeric | `ATH` |
| Charging Station ID | Unique station ID | 10 Alphanumeric | `umTDvSzocs` |
| Charging Point ID | Unique point ID | 10 Alphanumeric | `oPghrCd1o` |
| Connector ID | Unique connector ID | 14 Alphanumeric | `BGaz1wgWmijwoy` |

**Example:**

```
IND*ATH*umTDvSzocs*oPghrCd1o*BGaz1wgWmijwoy
```
...
```
beckn://discover?&message.catalogs.items.provider.id=atherenergy&message.catalogs.items.id=IND*PLS*umTDvSzocs*oPghrCd1o*BGaz1wgWmijwoy&version=1.0
```
